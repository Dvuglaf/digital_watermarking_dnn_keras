...
marked = keras.layers.Conv2D(3, (1, 1), padding='same', activation='sigmoid', input_shape=x.shape[1:])(x)
...
x = conv_block_b(x)
x = conv_block_b(x)
decoder_output = keras.layers.Conv2D(1, (1, 1), padding='same', name='5', activation='sigmoid', input_shape=x.shape[1:])(x)
...
marked = tf.Variable(initial_value=embedder_output)
top = network.get_layer('top')(marked)

mid = network.get_layer('mid_1')(marked)
mid = network.get_layer('mid_2')(mid)

bot = network.get_layer('bot_1')(marked)
bot = network.get_layer('bot_2')(bot)
bot = network.get_layer('bot_3')(bot)

b1m = network.get_layer('concat')([top, mid, bot])
b2m = network.get_layer('b2')(b1m)'''
...
x_train = x_train[:-48500]  # to 1920 images
y_train = y_train[:-48080]
x_test = x_test[:-9520]
y_test = y_test[:-9520]
x_train = x_train.reshape((1500, 32, 32, 1))
print(x_train.shape)

#   Coverimage (dataset from internet, 128x128x3)
# TODO: wrap into function
train = tf.keras.preprocessing.image_dataset_from_directory("C:/Users/vov4i/Desktop/dataset", image_size=(128, 128),
...
c_train = c_train.reshape((2600, 128, 128, 3))
res = c_train[:-1100]
print(res.shape)
